...
class GpioDev:
    ...
    def flush_input(self):
        ...
        try:
            while self.console.expect([pex.TIMEOUT, pex.EOF, '.*'],timeout=0.2) == 2:
                if len(self.console.after) == 0:
                    break
                _log.debug('[%s] In flush_input() got %s', self.name,
                       self.console.after)
        except Exception as e:
            _log.debug('[%s] Exception in flush_input: %s', self.name, str(e))
    ...
    def send_line(self, msg):
        ...
        try:
            ...
            _log.error('[%s] Failed to send line "%s": %s', self.name, msg, str(e))
...
def test_reset():
    ...
    if rc != 0:
        test_fail('Did not find pattern "%s" rc=%d' % (failed_pt, rc))
        passed = False
    else:
        test_pass()
    return passed
